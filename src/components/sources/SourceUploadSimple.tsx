...
interface SourceUploadSimpleProps {
  offeringId: string;
  maxSizeMb: number;
}

export default function SourceUploadSimple({ offeringId, maxSizeMb }: SourceUploadSimpleProps): JSX.Element {
  ...
  // Auto-focus textarea when modal opens in text mode
  useEffect(() => {
    if (open && mode === 'text') {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Use the API route directly with credentials (cookies)
